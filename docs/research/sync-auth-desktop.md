...
Apply coarse abuse controls to authenticated control-plane calls: initially 5 prepares per account per minute, 10 commits per project per minute, and 1,000 presigned-URL issuances per account per minute, with retry guidance and operator-configurable values. Those one-minute windows match the [Workers Rate Limiting binding](https://developers.cloudflare.com/workers/runtime-apis/bindings/rate-limit/), whose configured period is currently limited to 10 or 60 seconds. The binding is location-local, intentionally permissive, and eventually consistent, so it is suitable for abuse damping, not a global exact allowance. Exact concurrent-attempt, byte/file, and any future hourly/daily limits belong in D1. Also cap manifest parsing before allocating per-entry objects and make prepare/commit idempotent.
