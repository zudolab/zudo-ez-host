...
The operator follow-up lives in
...
standalone request. This harness is a post-deployment proof; it does not create
Cloudflare resources or deploy either Worker, and an unset gate prints
`SKIPPED, NOT RUN` rather than claiming success.
...
runs and has an independent verdict. Serving is **SKIPPED**, not passed, when
the exact public URL is not configured. See the
...
credential permissions, and the
[deployment runbook](../../../../doc/src/content/docs/hosting/deployment.mdx) for the ordered deploy and
second-pass staging setup.
